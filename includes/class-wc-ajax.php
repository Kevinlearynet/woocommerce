...
class WC_AJAX {
	...
	public function json_search_customer_name( $query ) {
		...
		$query_from  = " LEFT JOIN {$wpdb->usermeta} as meta2 ON ({$wpdb->users}.ID = meta2.user_id) ";
		$query_from  .= " LEFT JOIN {$wpdb->usermeta} as meta3 ON ({$wpdb->users}.ID = meta3.user_id) ";
		$query->query_from  .= apply_filters( 'woocommerce_json_search_customer_name_query_from', $query_from );

		$query_where = $wpdb->prepare( " OR ( meta2.meta_value LIKE %s OR meta3.meta_value LIKE %s )", '%' . like_escape( $term ) . '%', '%' . like_escape( $term ) . '%' );
		$query_where .= " AND meta2.meta_key = 'first_name' AND meta3.meta_key = 'last_name' ";
		$query->query_where  .= apply_filters( 'woocommerce_json_search_customer_name_query_where', $query_where );
	}
